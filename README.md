...
- AI 细解 / 让 ta 解牌：接你自己的接口（OpenAI 兼容 或 Anthropic），密钥只存在本机
- 图鉴：78 张牌正逆位的关键词和一句解
...
- 密钥、模型名
- 让谁解牌：ta 的名字，按钮上会写「讓 XX 解牌」
- 人设：写给 AI 的系统提示，ta 是谁、跟你什么关系、怎么说话

「AI 细解」是一个不认识你的塔罗师，只看牌，写得很细，按牌阵类型缓存，写过一次不再花钱。
「让 ta 解牌」把这次的牌（牌名、正逆位、位置、关键词、客观解读）连同人设发给 AI，回复存进这条记录。
...
js/ai.js              接 AI（OpenAI 兼容 / Anthropic，可走转发）
